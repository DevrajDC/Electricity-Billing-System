<?php 
  ...
  $consumer = $_GET["consumer_id"];
  $consumer_name = $_GET["consumer_name"];
  $set = $_GET['bill'];
  $res = getData($meter_id, $consumer); 
  $phone = $res["phone"];
  $email = $res["email"];
  $enb = $res["conn_status"] == 'Inactive' ? 'block' : 'none';
  $disb = $res["conn_status"] == 'Inactive' ? 'none' : 'block';
?>
          ...
          <div class="py-2 bg-white align-middle inline-block min-w-full sm:px-6 lg:px-8 shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <div class="relative pt-5 border-b border-gray-200 sm:pb-0">
              <div class="md:flex md:items-center md:justify-between">
                ...
                <button style="display: <?php echo $disb; ?>;" type="button" onclick="meterToggleForm('<?php echo $_GET['meter_num']; ?>', '<?php echo $_GET['consumer_name']; ?>', '<?php echo $_GET['consumer_id']; ?>', '<?php echo $res['conn_status']; ?>', 'Disable', 'Are you sure you want to Disable this Connection?'); toggleModal('Delete-modal')" class="inline-flex items-center px-2.5 py-1.5 border border-transparent rounded shadow-sm text-xs font-medium text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">Disable</button>
                <button style="display: <?php echo $enb; ?>;" type="button" onclick="meterToggleForm('<?php echo $_GET['meter_num']; ?>', '<?php echo $_GET['consumer_name']; ?>', '<?php echo $_GET['consumer_id']; ?>', '<?php echo $res['conn_status']; ?>' ,'Enable', 'Are you sure you want to Enable this Connection?'); toggleModal('Delete-modal')" class="inline-flex items-center px-2.5 py-1.5 border border-transparent rounded shadow-sm text-xs font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">Enable</button>
            ...
            <div>
              ...
              <div id="p1" class="mt-5 border-b border-gray-200" style="display: <?php echo $n; ?>;">
                <form action="../../LOGIC/admin/admin.php?action=updateUser&meter_num=<?php echo $meter_id; ?>&consumer_name=<?php echo $consumer_name; ?>&consumer_id=<?php echo $consumer; ?>" method="post">
                  ...
                  <dl class="divide-y divide-gray-200">
                    ...
                    <div class="py-4 sm:grid sm:py-5 sm:grid-cols-3 sm:gap-4">
                      ...
                      <dd class="mt-1 flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        <span class="flex-grow"><input name="consumer-phone" type="number" outline="none" value="<?php echo $res["phone"]; ?>" class="focus:outline-none focus:ring focus:border-indigo-600 rounded p-1 outline-offset-2 w-64" style="border:none;"></span>
                      </dd>
                    </div>
                    <div class="py-4 sm:grid sm:py-5 sm:grid-cols-3 sm:gap-4">
                      <dt class="text-sm font-medium text-gray-500">Email Address</dt>
                      <dd class="mt-1 flex text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        <span class="flex-grow"><input name="consumer-email" type="text" outline="none" value="<?php echo $res["email"]; ?>" class="focus:outline-none focus:ring focus:border-indigo-600 rounded p-1 outline-offset-2 w-64" style="border:none;"></span>
                      </dd>
                    </div>
                    </li>
                    </ul>
              ...
              <!-- Connection Tab -->
              <?php 
                $r = getConnDetails($meter_id);
              ?>
              <div id="p2" class="mt-5 border-b border-gray-200" style="display: none;">
                <form action="../../LOGIC/admin/admin.php?action=updateMeter&meter_num=<?php echo $meter_id; ?>&consumer_name=<?php echo $consumer_name; ?>&consumer_id=<?php echo $consumer; ?>" method="post">
                  <div class="w-full flex justify-between">
                        ...
                        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                          <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                              <?php displayBillsTab($meter_id,$consumer_name); ?>
                            </tbody>
                          </table>
                        </div>
                      </div>
...
<div class="fixed z-10 inset-0 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" id="Delete-modal" aria-modal="true">
  <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
    ...
    <div class="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
      <form action="../../LOGIC/admin/admin.php?action=toggleMeter" method="post">  
        ...
        <div class="mt-5 sm:mt-4 sm:ml-10 sm:pl-4 sm:flex">
          <input type="submit" value="Submit" class="inline-flex justify-center w-full rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:w-auto sm:text-sm">
